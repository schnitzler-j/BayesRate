...
if __name__=="__main__": 
	print """
              BayesRate 1.3 build 20120212
     Key-Innovation test (Moore and Donoghue 2009)
	"""
...
def MCMC():
	...
	while True:
		try: 
			...
			=parse_file(0, trees_ingroup, input_file, name_file, path_dir, 1)
			break
		except: print "\tFile format not recognized."
	...
	for i in range(len(dataset)): 
		if rhos[i]==0: head += "predicted_div_%s\tage_%s\t" % (name_clades[i], name_clades[i])
		else: head += "net_div_%s\text_frac_%s\t" % (name_clades[i], name_clades[i])
	for i in range(len(dataset)): 
		if rhos[i]==0: head += "null_sp\tnull_ext\t"
		else: head += "speciation_%s\textinction_%s\t" % (name_clades[i], name_clades[i])
	head += "tree"
	...
	for t in range(trees): # trees
		...
		for n in range(iteration): # generations
			...
			for d in range(n_datasets):
				...
				if rhos[d]==0 and prior>-inf: 
					lik=lik
					#temp_root.append(max(x))
				elif rhos[d]<1 and prior>-inf: 
					lik=lik+lnL_BD_Yang(x, lambdas[d], mus[d], models[d], rhos[d])
					#temp_root.append(max(x))
				elif rhos[d]==1 and prior>-inf: 
					lik=lik+lnL_BirthDeath(x, lambdas[d], mus[d], models[d])
				temp_root.append(max(x))
			...
			if n % print_freq ==0: 
				...
				print "\t r",lambdasA
				print "\t a", musA
				print "\t ages", temp_root
				print "\t no. sp.", exp(array(temp_root)*lambdasA[0])
			if n % sampling_freq ==0 and n> burnin: 
				IT += sampling_freq
				out="%s\t%s\t%s\t%s\t" % (IT, PostA, priorA, likA)
				list_coldlnL.append(likA)
				for i in range(len(dataset)): 
					if rhos[i]>0: out=out+ "%s\t%s\t" % (lambdasA[i], musA[i])
					else: 
						#print temp_root, lambdasA, i
						out=out+ "%s\t%s\t" % (exp(temp_root[i]*lambdasA[0]), temp_root[i])
						#print (exp(temp_root[i]*lambdasA[0]), temp_root[i])
